...
include=[] #['nifti'] # file types to include (optional, if empty will download all available)
...
for row in input_file:
    ...
    if not os.path.isdir('data/'+c_id): # if the subject's directory doesn't already exist
        ...
        session_ptr = fw.lookup(f'{fw_group_label}/{fw_proj_label}/{c_id}/{session}')

        if files==None or files==['']:
            session_ptr.download_tar(f'data/{c_id}.tar', include_types=include, exclude_types=exclude) # exclude_types=['dicom'] (optional, list)
            ...
            cntr=0
            for (path, __, __) in os.walk(f'data/{c_id}'):
                if cntr==1:
                    inst_path=path
                elif cntr==2:
                    grp_path=path
                    ...
                    shutil.move(path, f'data/{c_id}') # move session directory up to main subject dir
                cntr=cntr+1
            os.rmdir(subj_path)
            os.rmdir(proj_path)
            os.rmdir(grp_path)
            ...
            cntr=0
            if 'dicom' in include:
                for (path, __, file_names) in os.walk(f'data/{c_id}'):
                    if cntr>1:
                        thispath=path
                        thispath+='/'
            ...
            if not os.path.isdir(f'data/{c_id}'):
                os.makedirs(f'data/{c_id}/{session}')
            ...
            for file in files:
                ...
                if session_ptr.acquisitions.find_first(f'label={acq_label}'):
                    ...
                    acq_ptr.download_file(file, f'data/{c_id}/{session}/{file}')
        print('Done: '+c_id)
